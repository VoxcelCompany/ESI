...
const addDevoir = async (params) => {
    ...
    if (getCurrentDate(date, 'DD/MM/YYYY', true).isValid()) {
        (client.channels.cache.get(process.env.CHNL_ALERT)).send(`<@${interaction.user.id}> (${interaction.user.id}) | Voudrait ajouter un devoir pour le \`${date}\` en ${matiere} \`${desc}\``)

        return await interaction.editReply({
            content: `<:check:866581082551615489> Votre devoir est en attente de validation par <@${process.env.USR_NKW}>, merci beaucoup !\nDate: \`${date}\`, matière: \`${matiere}\`, détails: \`${desc}\``
        }).catch(() => { });
    } else {
        return await interaction.editReply({

...
const edt = async (params) => {
    ...
    if (num == '1') {
        ...
        if ((!edt[edtDate] || edt[edtDate].myges == 'UNDEFINED') && type == "BUTTON") {
            return (client.channels.cache.get(process.env.CHNL_ALERT)).send(`<@${process.env.USR_NKW}> | <@${interaction.user.id}> waits EDT 1 ${datefinale}\n\`&edt sendmp ${datefinale} ${interaction.user.id} 1\``).catch(() => { })
        } else {
            let pastille;
            let myges;
        ...
        const edt = await getAllData("edt")
        if ((!edt[edtDate] || edt[edtDate].myges == 'UNDEFINED') && type == "BUTTON") {
            return (client.channels.cache.get(process.env.CHNL_ALERT)).send(`<@${process.env.USR_NKW}> | <@${interaction.user.id}> waits EDT 2 ${datefinale}\n\`&edt sendmp ${datefinale} ${interaction.user.id} 1\``).catch(() => { })
        } else {
            let pastille;
            let myges;
        ...
        if ((!edt[edtDate] || edt[edtDate].myges == 'UNDEFINED') && type == "BUTTON") {
            return (client.channels.cache.get(process.env.CHNL_ALERT)).send(`<@${process.env.USR_NKW}> | <@${interaction.user.id}> waits EDT 3 ${datefinale}\n\`&edt sendmp ${datefinale} ${interaction.user.id} 1\``).catch(() => { })
        } else {
            let pastille;
            let myges;
